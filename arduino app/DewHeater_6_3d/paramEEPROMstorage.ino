...
bool checkParamError(int theVal, int minVal, int maxVal, int incrVal) {
  ...
  errorVal = true;
  // check parameters in valid = in the range of allowed values & follows the increment, e.g. manualPower = 0 - 100% in increments of 5%
...
bool checkErrorEEPROMdata() {
  ...
  // check EEPROM value, an error if
  // - not a number, OR
  // - within the allowed range & at their increment. Values from arrays: 0=min 1=max 2=default 3=incr

  // global mode
  if ( (globalMode != globalMode) || (checkParamError( globalMode, globalModeRange[0], globalModeRange[1], globalModeRange[3])) )
  {
    globalMode = globalModeRange[2];    // reset to default
    errorParam = true;      // is a number but not in range
  }

  // manual power
  if ( (globalMode != globalMode) || (checkParamError( manualPower, manualPowerRange[0], manualPowerRange[1], manualPowerRange[3])) )
  {
    manualPower = manualPowerRange[2];
    errorParam = true;
  }

  // max power for auto-ambient mode
  if ( (aAmbMaxPwr != aAmbMaxPwr) || (checkParamError( aAmbMaxPwr, aAmbMaxPwrRange[0], aAmbMaxPwrRange[1], aAmbMaxPwrRange[3])) ) {     
    aAmbMaxPwr = aAmbMaxPwrRange[2];
    errorParam = true;
  }

  // threshold temp for auto-ambient, relative to dew point
  if ( (aAmbThresh != aAmbThresh) || (checkParamError( aAmbThresh, aAmbThreshRange[0], aAmbThreshRange[1], aAmbThreshRange[3])) ) {     
    aAmbThresh = aAmbThreshRange[2];
    errorParam = true;
  }

  // target heater temp for auto-heater mode, relative to dew point
  if ( (aHtrTargetTemp != aHtrTargetTemp) || (checkParamError( aHtrTargetTemp, aHtrTargetTempRange[0], aHtrTargetTempRange[1], aHtrTargetTempRange[3])) ) {
    aHtrTargetTemp = aHtrTargetTempRange[2];
    errorParam = true;
  }

  // temperature cut-off
  if ( (tempCutOff != tempCutOff) || (checkParamError( tempCutOff, tempCutOffRange[0], tempCutOffRange[1], tempCutOffRange[3])) ) {     
